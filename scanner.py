...
def process_frame(frame, geo_data):
    ...
    results = model.predict(frame, imgsz=1280, conf=0.5)
    image_height_y, image_width_x, c = frame.shape
    print(image_width_x, image_height_y)
    # List to store each object's data
    detected_objects = []

    for result in results:
        for box in result.boxes:
            px_x, px_y, w, h = box.xywh[0].cpu().numpy()  # Extract bounding box coordinates and size
...
def display_frame(frame, speed, bearing, current_altitude, objects=[]):
    ...
    for obj in objects:
        ...
        coord_text = f"Lon: {longitude:.6f}, Lat: {latitude:.6f}"
...
if __name__ == "__main__":
    ...
    model = YOLO("/Users/alainfrey/Library/CloudStorage/GoogleDrive-alain.frey@outlook.com/My Drive/seg1920-200-customcombined-aug-fullsize/train/weights/best.pt")  # Adjust the model path and name as necessary
    #model = YOLO("/Users/alainfrey/Library/CloudStorage/GoogleDrive-alain.frey@outlook.com/My Drive/detect1280b16ep200custom138/train/weights/best.pt")  # 

    # Load SRT file
    subs = pysrt.open('/Users/alainfrey/Lichtwinkel Dropbox/Alain Frey/thesis/strandkirkja/good/DJI_0208.srt')
    ...
    #/Users/alainfrey/Lichtwinkel Dropbox/Alain Frey/thesis/westman/DJI_0301.MP4
    #/Users/alainfrey/Lichtwinkel Dropbox/Alain Frey/thesis/shrimp/DJI_0245.MP4
    frame_id = 0
        ...
        while True:
            ...
            if sub and sub.text.strip():
                ...
                if len(coords_window) >= 2:
                    ...
                    altitude_m = geo_data['rel_alt']

                    image_width_x, image_height_y, c = frame.shape

                    gsd_x, gsd_y = location2.calculate_gsd(sensor_dimension_x_mm, sensor_dimension_y_mm, altitude_m, image_width_x, image_height_y, focal_length_mm)

                    # Display the frame with speed and bearing
                    if speed != 0:
                        #skip_frames = calculate_frames_to_skip(speed, gsd, image_height, fps)
                        skip_frames = 5
                    else:
                        skip_frames = 5
                    #print(skip_frames)
